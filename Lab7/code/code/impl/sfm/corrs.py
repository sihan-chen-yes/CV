...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  images[keys[0]].Add3DCorrs(corrs[keys[0]], [offset + idx for idx in list(range(len(new_points3D)))])
  for im_name in keys[1:]:
    cur_len = len(corrs[im_name])
    images[im_name].Add3DCorrs(corrs[im_name], [offset + idx for idx in list(range(cur_len))])
    offset += cur_len
